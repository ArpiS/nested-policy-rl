...
if __name__ == "__main__":
    ...
    for i in range(num_iter):
        ...
        # printed_bg, printed_fg, performance, nfq_agent, X = run(is_contrastive=False, init_experience_bg=20, init_experience_fg=20, fg_only=True, force_left=4, epoch=2000, verbose=False)
        # results['fg_only'].extend(performance)

        # Use contrastive model with larger pool of background samples
        # printed_bg, printed_fg, performance, nfq_agent, X = run(is_contrastive=True, init_experience_bg=200, init_experience_fg=40, fg_only=False, force_left=4, epoch=2000, verbose=False)
        # results['cfqi'].extend(performance)

        # Use non-contrastive model with larger pool of background samples
        printed_bg, printed_fg, performance, nfq_agent, X = run(is_contrastive=False, init_experience_bg=200, init_experience_fg=40, fg_only=False, force_left=4, epoch=2000, verbose=True)
