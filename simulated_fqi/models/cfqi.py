...
#             (np.asarray(batch["s"]), np.expand_dims(np.asarray(batch["a"]), 1))
        ...
        y_fg = batch_foreground["r"] + (
            self.gamma * np.max(Q[batch_foreground["ns_ids"], :], axis=1)
        )
    ...
    def updateQtable(self, Qtable, batch_fg, batch_bg):
        ...
        for i, a in enumerate(self.unique_actions):
            Qtable[batch_bg["s_ids"], i] = self.q_est_shared.predict(
                np.hstack((batch_bg["ns"], np.tile(a, (bg_size, 1))))
            )
            Qtable[batch_fg["s_ids"], i] = self.q_est_fg.predict(
